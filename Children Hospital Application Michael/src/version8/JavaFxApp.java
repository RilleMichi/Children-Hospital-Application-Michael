...
public class JavaFxApp extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		// Hier werden die Events implementiert. hier werden die Tavs nachdem suchbegriff gesucht
		searchfield.setOnKeyTyped(EventHandler -> {
			String searString = searchfield.getText();
			filterTab(tab1, this.model.getAllPatients(), searString);
			filterTab(tab2, this.model.getParents(), searString);
			filterTab(tab3, this.model.getPediatricians(), searString);
		});
		...
		inputVaccinationItem.setOnAction(event -> {
			// übergebe tab vom patient
			this.enterVacination(stage, tab1);
		});
		...
		aboutItem.setOnAction(event -> {
			...
			alert.setContentText("This is a JavaFx Application in Version 8");
		...
		VBox vBox = new VBox(menuBar, tabPane, searchfield);
	...
	// Tabs aktualisieren
	public static <T> void updateTab(Tab tab, Collection<T> collection) {
	...
	// Filter
	public static <T> void filterTab(Tab tab, Collection<T> collection, String searchString) {
		TextArea textArea = (TextArea) tab.getContent();
		// setze den Text wieder leer, damit es nicht doppelt angezeigt wird.
		textArea.setText("");
		for (T element : collection) {
			//Wenn es den String enthält, dann gebe diese aus
